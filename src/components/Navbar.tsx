...
export const navigationItems = [
  ...
  },
  {
    ...
    icon: <HeartOutlined />,
    title: '宠物管理',
    description: '定制你的宠物名字、天气和场景，打造专属小世界。',
    actionText: '去装扮',
    tags: ['个性化', '天气', '场景'],
    themeColor: 'rose',
...
function Sidebar({ onLoginOpen }: { onLoginOpen: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex items-center h-[52px] flex-shrink-0 ${collapsed ? 'justify-center px-0' : 'justify-between px-4'}`}
      >
        <Link
          href="/"
          className="text-xl font-bold tracking-tight whitespace-nowrap block"
        >
      ...
      <nav className="flex-1 flex flex-col gap-0.5 px-2 pt-2">
        ...
        >
          <span
            className={`text-base flex-shrink-0 ${pathname === '/' ? 'text-indigo-500 dark:text-indigo-400' : ''}`}
          >
            <HomeOutlined />
          </span>
        ...
        {navigationItems
          ...
          return (
            ...
            >
              <span
                className={`text-base flex-shrink-0 ${isActive ? 'text-indigo-500 dark:text-indigo-400' : ''}`}
              >
                {item.icon}
              </span>
              <AnimatePresence initial={false}>
      ...
      {/* Bottom — 主题 & 登录 & 收起 */}
      <div
        className={`pb-5 flex flex-col gap-1 px-2 overflow-hidden ${collapsed ? 'items-center' : ''}`}
      >
        {/* 主题切换 */}
        <button
